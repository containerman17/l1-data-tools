...
// StrictUnmarshal unmarshals JSON with DisallowUnknownFields to catch missing struct fields
func StrictUnmarshal(data []byte, v any) error {
	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(v); err != nil {
		return fmt.Errorf("strict unmarshal failed: %w", err)
	}
	return nil
}
...
// ChainConfig for RPC fetcher
type ChainConfig struct {
	ChainID        uint64
	Name           string
	URL            string
	MaxParallelism int // Default: 200
	MaxLatencyMs   int // Max P95 latency before reducing parallelism. Default: 1000
}
