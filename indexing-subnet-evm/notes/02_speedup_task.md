...
| `evm-ingestion/storage/pebble.go` | Rename to PebbleStorage |
| `evm-ingestion/storage/versiondb.go` | NEW - VersionDBStorage |
| `evm-ingestion/storage/compactor.go` | Use Storage interface |
| `evm-ingestion/api/server.go` | Use Storage interface |
| `evm-ingestion/main.go` | Use NewPebbleStorage |
| `indexing-subnet-evm/vm.go` | Add getVersionDB(), use VersionDBStorage |
| `indexing-subnet-evm/block.go` | Fix lastIndexedHeight timing |
| `indexing-subnet-evm/indexer.go` | Remove lastIndexedHeight.Store() |
