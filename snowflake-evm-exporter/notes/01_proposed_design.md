...
A CLI tool that subscribes to the `evm-ingestion` WebSocket service, transforms `NormalizedBlock` data into Snowflake-compatible format, and uploads to 5 target tables atomically using Snowflake transactions.
...
└─────────────────────┘     └──────────────────────┘     └─────────────────┘
        │                            │                           │
   NormalizedBlock              Transform &               5 Tables:
...
| Extracted from Logs/Input | `icm_messages` | msg_id, source_chain, dest_chain, payload, etc. |
...
type ExportBatch struct {
    ...
    ICMMessages  []ICMMessageRow  // May be empty
}
```
...
INSERT INTO icm_messages VALUES (...), (...), ...;  -- May be empty

COMMIT;
```

All 5 inserts succeed together or all rollback.
...
1. **Export to CSV:**
   ...
   ```
   Produces: `blocks.csv`, `transactions.csv`, `logs.csv`, `internal_txs.csv`, `icm_messages.csv`
...
## ICM Message Extraction

### Challenge
ICM (Interchain Messaging) data is embedded in logs and/or transaction input data. Not all chains have ICM.

### Approach
1. Detect ICM-capable chains by chain ID
2. Filter logs by known ICM contract addresses
3. Parse ICM-specific event signatures
4. Extract message payload from log data or tx input

**Note:** This requires knowledge of ICM contract addresses and event ABIs. May need chain-specific configuration.
...
### Phase 2: Internal Transactions & ICM
- [ ] Trace flattening for internal transactions
- [ ] ICM message extraction (chain-specific)
- [ ] Full 5-table atomic writes
