...
CLI tool that downloads EVM chain data from the `evm-ingestion` service and exports it to Snowflake. Designed to run once a day (or more frequently) as a catch-up process.

---

## Data Source

- Consumes data from `evm-ingestion` via WebSocket subscription
